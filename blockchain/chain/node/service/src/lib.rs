...
pub use catena_client::CatenaExecutorDispatch;

pub use chain_spec::CatenaChainSpec;
pub use consensus_common::{block_validation::Chain, Proposal, SelectChain};
#[cfg(feature = "full-node")]
pub use catena_client::{
...
> {
	...
	#[cfg(feature = "catena-native")]
	{
		return chain_ops!(config, jaeger_agent, telemetry_worker_handle; catena_runtime, CatenaExecutorDispatch, Catena)
	}
...
pub fn build_light(config: Configuration) -> Result<(TaskManager, RpcHandlers), Error> {
	...
	#[cfg(feature = "catena-native")]
	{
		return new_light::<catena_runtime::RuntimeApi, CatenaExecutorDispatch>(config)
	}
...
pub fn build_full(
	...
	#[cfg(feature = "catena-native")]
	{
		return new_full::<catena_runtime::RuntimeApi, CatenaExecutorDispatch, _>(
			config,
			is_collator,
			grandpa_pause,
			disable_beefy,
		...
		.map(|full| full.with_client(Client::Catena))
	}
