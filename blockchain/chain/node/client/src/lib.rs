...
pub struct CatenaExecutorDispatch;

impl sc_executor::NativeExecutionDispatch for CatenaExecutorDispatch {
	type ExtendHostFunctions = frame_benchmarking::benchmarking::HostFunctions;

	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		catena_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		catena_runtime::native_version()
	}
}
...
#[derive(Clone)]
pub enum Client {
	Catena(Arc<FullClient<catena_runtime::RuntimeApi, CatenaExecutorDispatch>>),
}
