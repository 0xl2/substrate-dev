...
impl SubstrateCli for Cli {
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		Ok(match id {
			"" => Box::new(service::chain_spec::polkadot_config()?),
			#[cfg(feature = "catena-native")]
			"develop" => Box::new(service::chain_spec::polkadot_development_config()?),
			#[cfg(feature = "catena-native")]
			...
			path => {
				...
				Box::new(service::CatenaChainSpec::from_json_file(path.clone())?) as Box<dyn service::ChainSpec>
			}
		})
	}
	...
	fn native_runtime_version(spec: &Box<dyn service::ChainSpec>) -> &'static RuntimeVersion {
		...
		#[cfg(feature = "catena-native")]
			{
				return &service::catena_runtime::VERSION;
			}
	}
}
...
fn set_default_ss58_version(_: &Box<dyn service::ChainSpec>) {
	...
	let ss58_version = Ss58AddressFormat::Custom(1020);
	sp_core::crypto::set_default_ss58_version(ss58_version);
}

const DEV_ONLY_ERROR_PATTERN: &'static str =
	"can only use subcommand with --chain [develop], got ";
	...
	match &cli.subcommand {
		...
		Some(Subcommand::Benchmark(cmd)) => {
			...
			#[cfg(feature = "catena-native")]
				{
					return Ok(runner.sync_run(|config| {
						cmd.run::<service::catena_runtime::Block, service::CatenaExecutorDispatch>(
							config,
						)
							.map_err(|e| Error::SubstrateCli(e))
					})?);
				}
			#[cfg(not(feature = "catena-native"))]
			panic!("No runtime feature (catena, polkadot, kusama, westend, rococo) is enabled")
		}
			...
			#[cfg(feature = "catena-native")]
				{
					return runner.async_run(|config| {
						Ok((
							cmd.run::<service::catena_runtime::Block, service::PolkadotExecutorDispatch>(
								config,
							)
								.map_err(Error::SubstrateCli),
							task_manager,
