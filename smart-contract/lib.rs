...
mod TenpNFT {
    ...
    mod tests {
        ...
        fn mint_works() {
            ...
            let mut tenp_nft = TenpNFT::new();
            // Token 1 does not exists.
            assert_eq!(tenp_nft.owner_of(1), None);
            // Alice does not owns tokens.
            assert_eq!(tenp_nft.balance_of(accounts.alice), 0);
            // Create token Id 1.
            assert_eq!(tenp_nft.mint(1), Ok(()));
            // Alice owns 1 token.
            assert_eq!(tenp_nft.balance_of(accounts.alice), 1);
        }
        ...
        fn mint_existing_should_fail() {
            ...
            assert_eq!(tenp_nft.owner_of(1), Some(accounts.alice));
            // Cannot create  token Id if it exists.
            // Bob cannot own token Id 1.
            assert_eq!(tenp_nft.mint(1), Err(Error::TokenExists));
        }
        ...
        fn transfer_works() {
            ...
            assert_eq!(tenp_nft.balance_of(accounts.bob), 0);
            // The first Transfer event takes place
            assert_eq!(1, ink_env::test::recorded_events().count());
            // Alice transfers token 1 to Bob
            assert_eq!(tenp_nft.transfer(accounts.bob, 1), Ok(()));
            // The second Transfer event takes place
            assert_eq!(2, ink_env::test::recorded_events().count());
            // Bob owns token 1
            assert_eq!(tenp_nft.balance_of(accounts.bob), 1);
        }
        ...
        fn invalid_transfer_should_fail() {
            ...
            assert_eq!(tenp_nft.transfer(accounts.bob, 2), Err(Error::TokenNotFound));
            // Token Id 2 does not exists.
            assert_eq!(tenp_nft.owner_of(2), None);
            // Create token Id 2.
            assert_eq!(tenp_nft.mint(2), Ok(()));
            // Alice owns 1 token.
            assert_eq!(tenp_nft.balance_of(accounts.alice), 1);
            // Token Id 2 is owned by Alice.
            assert_eq!(tenp_nft.owner_of(2), Some(accounts.alice));
            ...
            assert_eq!(tenp_nft.transfer(accounts.eve, 2), Err(Error::NotApproved));
        }
        ...
        fn approved_transfer_works() {
            ...
            assert_eq!(tenp_nft.approve(accounts.bob, 1), Ok(()));
            // Get contract address.
            let callee = ink_env::account_id::<ink_env::DefaultEnvironment>()
                .unwrap_or_else(|_| [0x0; 32].into());
            // Create call
            ...
            assert_eq!(
                tenp_nft.transfer_from(accounts.alice, accounts.eve, 1),
                Ok(())
            );
            // TokenId 3 is owned by Eve.
            assert_eq!(tenp_nft.owner_of(1), Some(accounts.eve));
            // Alice does not owns tokens.
            assert_eq!(tenp_nft.balance_of(accounts.alice), 0);
            // Bob does not owns tokens.
            assert_eq!(tenp_nft.balance_of(accounts.bob), 0);
            // Eve owns 1 token.
            assert_eq!(tenp_nft.balance_of(accounts.eve), 1);
        }
        ...
        fn approved_for_all_works() {
            ...
            assert_eq!(tenp_nft.mint(2), Ok(()));
            // Alice owns 2 tokens.
            assert_eq!(tenp_nft.balance_of(accounts.alice), 2);
            // Approve token Id 1 transfer for Bob on behalf of Alice.
            assert_eq!(tenp_nft.set_approval_for_all(accounts.bob, true), Ok(()));
            // Bob is an approved operator for Alice
            assert!(tenp_nft.is_approved_for_all(accounts.alice, accounts.bob));
            // Get contract address.
            let callee = ink_env::account_id::<ink_env::DefaultEnvironment>()
                .unwrap_or_else(|_| [0x0; 32].into());
            // Create call
            ...
            assert_eq!(
                tenp_nft.transfer_from(accounts.alice, accounts.eve, 2),
                Ok(())
            );
            // Bob does not owns tokens.
            assert_eq!(tenp_nft.balance_of(accounts.bob), 0);
            // Eve owns 2 tokens.
            assert_eq!(tenp_nft.balance_of(accounts.eve), 2);
            ...
            assert_eq!(tenp_nft.set_approval_for_all(accounts.bob, false), Ok(()));
        ...
        fn not_approved_transfer_should_fail() {
            ...
            assert_eq!(tenp_nft.balance_of(accounts.eve), 0);
            // Get contract address.
            let callee = ink_env::account_id::<ink_env::DefaultEnvironment>()
                .unwrap_or_else(|_| [0x0; 32].into());
            // Create call
            ...
            assert_eq!(
                tenp_nft.transfer_from(accounts.alice, accounts.frank, 1),
        ...
        fn burn_fails_token_not_found() {
            ...
            assert_eq!(tenp_nft.burn(1), Err(Error::TokenNotFound));
        }
        ...
        fn burn_fails_not_owner() {
            ...
            assert_eq!(tenp_nft.burn(1), Err(Error::NotOwner));
        }
